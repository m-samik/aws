...
os.system("aws ec2 run-instances --image-id ami-0ad704c126371a549 --instance-type t2.micro  --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=Iamfromcli}]'  --key-name myawskey  > instance.json")
...
os.system("aws ec2 create-volume --volume-type gp2   --availability-zone {0} --size 5  > c_volume.json".format(region))
